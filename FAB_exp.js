...
// fix the place of the stimuli at each trial
...
let vec = [0 ,1, 2, 3];
vec = jsPsych.randomization.shuffle(vec); // randomize the place of each stimulus
...
let ms_prac = Array(4).fill(0); // mean reward of each option at practice
...
const get_exp_num = {
    ...
    on_finish: function(data) {
        ...
        for (let k = 0; k < 4; k++) {
            ...
            ms_prac[k] = reward_mean(trial_n, step_size, incr_size, lambda, cv_nu, participantID, exp_num);
        };
    }
};
...
const show_outcome = {
    ...
    stimulus: function() {
        ...
        let true_m;
        if (prac_or_main == "prac") {
            true_m = [ms_prac[0][total_tn], ms_prac[1][total_tn], ms_prac[2][total_tn], ms_prac[3][total_tn]];
        } else {
            true_m = [ms[0][total_tn], ms[1][total_tn], ms[2][total_tn], ms[3][total_tn]];
        }
...
const end_exp = {
    ...
    stimulus: "<p class='inst_text'>これで実験は終了です。お疲れさまでした。<br>「実験後の質問紙」に回答するのを忘れないようにお願いいたします。<br><b>スペースキーを押して，"  +
        "必ず画面が完全に真っ暗になったのを確認してから</b>，画面を閉じてください。<br>見えにくいですが，灰色のローディングマークが中央で回っている間はデータ転送中ですので，画面を消さないでください。" +
        "<br>通常は30秒ほどでデータ転送が完了します。</p>",
    choices: [" "]
};
